...
-- EMPv0 뷰 생성 : ENAME, JOB, HIREDATE만으로 뷰 (NOT NULL필드를 미포함하면 INSERT 불가)
CREATE OR REPLACE VIEW EMPv0
...
SELECT * FROM EMPv0;
INSERT INTO EMPv0 VALUES (1111, 'HONG', 'MANAGER', 'RESEARCH', 'DALLAS'); -- 복합뷰는 갱신 불가
-- ② 가상의 필드로 뷰 : 컬럼에 별칭(영문자로시작, 영문자, 숫자, _)을 이용하여 뷰 생성
CREATE OR REPLACE VIEW EMPv0
    AS SELECT EMPNO, ENAME, SAL*12 "YEAR_SAL" FROM EMP WHERE DEPTNO=10; -- 기존의 별칭

CREATE OR REPLACE VIEW EMPv0 (NO, ENAME, YEAR_SAL)        -- 별칭들만 따로 나열
...
-- 2. INLINE VIEW ; FROM절 상의 서브쿼리를 INLINE VIEW라 하며, FROM절에 오는 서브쿼리는 VIEW처럼 작용
...
SELECT ROWNUM, ENAME, SAL 
    FROM (SELECT ENAME, SAL FROM EMP ORDER BY SAL DESC)
    WHERE ROWNUM  BETWEEN 6 AND 10; -- 결과가 한행 안 나옴
-- ★ TOP - N 구문 1단계
SELECT ROWNUM RN, ENAME, SAL FROM (SELECT ENAME, SAL FROM EMP ORDER BY SAL DESC);
-- ★ TOP - N 구문 2단계(완성된 TOP-N구문)
SELECT ROWNUM, RN, ENAME, SAL
    FROM (SELECT ROWNUM RN, A.* FROM (SELECT ENAME, SAL FROM EMP ORDER BY SAL DESC) A) 
...
SELECT EMPNO, ENAME, HIREDATE
    ...
    WHERE RN BETWEEN 6 AND 10; -- 6번째부터 10번째 사원
