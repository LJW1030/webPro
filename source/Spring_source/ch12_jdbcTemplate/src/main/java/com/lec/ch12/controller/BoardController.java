...
// mvcboard/list.do, mvcboard/write.do, ...
...
public class BoardController {
	...
	public void setTemplate(JdbcTemplate template) {
		...
		System.out.println("template 가져옴");
	}
	...
	public String content(int bid, Model model) {
		...
		bservice = new BContentService(); // 조회수 1올리고 dto 가져오기
		bservice.execute(model);
	...
	public String content_post(int bid, Model model) {
		model.addAttribute("bid", bid);
		bservice = new BModifyReplyViewService(); // 조회수를 올리지않고 dto 가져오기
		bservice.execute(model);
		return "mvcboard/content";
	}
